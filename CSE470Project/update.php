		...
		if($new_crop_name === ""){
			...
			if(!$connection){
				...
				if(exists($crop_name_array,strtolower($new_crop_name))){
					...
					$query2 = "select distinct name from bangladesh_crops;";
					$result2 = mysqli_query($connection,$query2);
					$crop_name_array = array();
					while($row2 = mysqli_fetch_assoc($result2)){
						$crop_name2 = $row2["name"];
					...
					if(!exists($crop_name_array,strtolower($new_crop_name))){
						$new_crop_confirmation_message = ucfirst($new_crop_name)." is not in Bangladesh Crops. Please contact the Admin";
						$new_crop_confirmation_message_display = "block";
					}else{
