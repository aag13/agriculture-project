<?php 
	...
	if(isset($_POST["submit"])){
		...
		if(isset($_POST["crop_name"])){
			...
			$crop_name_array = get_bangladesh_crop();
			$crop_name_array_count = count($crop_name_array);
		}
		...
		if(isset($_POST["year"])){
			...
			$year_array = get_bangladesh_year();
			$year_array_count = count($year_array);
		}
		...
		foreach($year_array as $yr){
			$query_array = bangladesh_query_generator1($query_array, $yr, $crop_name_array, $crop_attribute_array);
		}
		...
		<div class = "bodycenter">
			...
			<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" >
				...
				<select name="crop_attribute[]" id="crop_attribute" multiple>
					...
					<option value="price" <?php if(exists($crop_attribute_array,"price")){echo " selected=\"selected\"";} ?> >Average Price </option>
					<option value="cultivated_land" <?php if(exists($crop_attribute_array,"cultivated_land")){echo " selected=\"selected\"";} ?> >Total Cultivated Land </option>
					<option value="estimated_production" <?php if(exists($crop_attribute_array,"estimated_production")){echo " selected=\"selected\"";} ?> >Total Estimated Production </option>
			...
			<h2><?php echo ucfirst($division) ?> Crop Information: </h2>
			
			
			
			<!--  -----------------To Generate Graph------------------------------
			<div>
			<form style="display:<?php echo $disp ?>">
			Year:
				<select name="year" id="year" >
					
					<?php 
						echo "<option value=\"\" selected=\"selected\">---</option>";
						foreach($year_array as $yy){
							echo "<option value=\"$yy\" >". ucfirst($yy) ."</option>";
						}
					?> 
				</select>
				...
				<select name="crop" id="crop" >
					...
					<?php 
						echo "<option value=\"\" selected=\"selected\"> --- </option>";
						foreach($crop_name_array as $cnn){
							echo "<option value=\"$cnn\" >". ucfirst($cnn) ."</option>";
						}
					?>
				</select>
				<p> to highlight rows by year or by crop </p>
			...
			<?php 
				...
				// if more than 2 years is in the table, it gets too long,, so break it up
				...
				if(count($query_array) == 0){
					...
					if(!$connection){
						...
						foreach($crop_attribute_array as $c_attr){
							echo "<th class=\"th_1\">".bangladesh_col_name($c_attr)."</th>";
						}
						echo "</tr>";
							...
							if($result){
								...
								if(mysqli_num_rows($result) != 0){
									...
									while($row = mysqli_fetch_assoc($result)){
										...
										if(isset($row["AveragePrice"])){
											echo "<td style=\"background-color:#{$temp_color}\" > <strong>".$row["AveragePrice"]."</strong> </td>";
										}
										if(isset($row["TotalCultivatedLand"])){
											echo "<td style=\"background-color:#{$temp_color}\" > <strong>".$row["TotalCultivatedLand"]."</strong> </td>";
										}
										if(isset($row["TotalEstimatedProduction"])){
											echo "<td style=\"background-color:#{$temp_color}\" > <strong>".$row["TotalEstimatedProduction"]."</strong> </td>";
										}
